...
{	
	...
	public class DeploymentFileCompilerTask : Microsoft.Build.Utilities.Task
	{
        ...
        /// <summary>
        /// (Optional) Should we copy items under the Misc items in project to
        /// the output folder. 
        /// </summary>
        public bool IncludeMiscItems { get; set; } = false;

        /// <summary>
        /// A space separated list of file extensions that will be copied feom the
        /// Misc items folder. We default to DLLs by default as that's our use case
        /// and we want to ignore any BIML files that may also be located under misc items
        /// </summary>
        public string IncludeExtensions { get; set; } = "dll";
			...
			foreach (var projectFile in InputProject)
			{
				try
				{
					...
					if (VsProject.DeploymentModel == DeploymentModel.Project)
					{
                        ...
                        // CUT IN HERE AND CREATE A MISC FOLDER
                        // HOLDING THE MISC ITEMS FROM THE PROEJCT
                        // Test Code (show the number of misc items in the SSIS project)
                        if (IncludeMiscItems)
                        {
                            // split the string into list 
                            List<String> extensionsToInclude = IncludeExtensions.Split(' ').ToList();

                            if (extensionsToInclude.Count > 0)
                            { 

                                Log.LogMessage("IncludeMiscItems Value: {0}",IncludeMiscItems.ToString());
                                Log.LogMessage("IncludeExtensions Value: {0}", IncludeExtensions);
                                Log.LogMessage("Found {0} Misc items",VsProject.Miscellaneous.Count);

                                foreach (Microsoft.AnalysisServices.Project.ProjectItem miscItem in VsProject.Miscellaneous)
                                {

                                    string fileExtension = Path.GetExtension(miscItem.FullPath).Substring(1);

                                    if (extensionsToInclude.Contains(fileExtension))
                                    {
                                        // check that the extension of the current misc item file
                                        // is in the list of extensions to include

                                        // If so copy the file

                                        // put exception handling around the file in case it doesnt exist
                                        //File.Copy(miscItem.FullPath, outputDirectory);

                                        Log.LogMessage("Coping Misc Item: {0}", miscItem.FullPath);

                                        Log.LogMessage("projectOutputPath: {0}", projectOutputPath);

                                        File.Copy(Path.Combine(projectDirectory,miscItem.FullPath),    // source
                                                  Path.Combine(projectOutputPath,miscItem.FullPath));  // dest

                                    }
                                    else
                                        Log.LogMessage("Skipping Misc Item {0} ", miscItem.FullPath);

                                }
                            }
                            else
                            {
                                Log.LogWarning("IncludeMiscItems is enabled, but unable to parse IncludeExtensions parameter. No Misc Items will be copied to the output folder");
                            }
                        } // END IncludeMiscItems
